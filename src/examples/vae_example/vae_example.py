...
class VAEExample(Model):
    def __init__(self, dataset, hparams):
        ...
        self.prior = tfd.Independent(
            tfd.Normal(loc=tf.zeros((self.hparams["latent_size"])), scale=1),
            reinterpreted_batch_ndims=1,
        )
        self.negloglik = lambda x, rv_x: -rv_x.log_prob(x)
        self.vae, self.encoder, self.decoder = create_vae(
            self.hparams["latent_size"],
            self.hparams["model_scale"],
            self.hparams["window_samples"],
            self.prior,
        )
    ...
    def train(self):
        ...
        self.vae.fit(
            ...
            validation_data=self.dataset.take(100),
