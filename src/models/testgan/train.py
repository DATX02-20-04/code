...
def start(hparams):
    ...
    testgan_stats = calculate_dataset_stats(hparams, dataset)

    dataset = nsynth_to_melspec(dataset, hparams, testgan_stats)
...
def calculate_dataset_stats(hparams, dataset):
    ...
    np.savez('testgan_stats.npz', mean=mean, min=min_, max=max_, variance=variance)
