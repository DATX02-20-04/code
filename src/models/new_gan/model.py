...
class GAN(tfk.Model):
    ...
    def create_train_step(self, generator, discriminator, half_batch):
        ...
        def apply_grad(X_real, y_real_aux):
            with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
                ...
                d_total_loss = (d_real_loss + d_fake_loss) * self.hparams['src_loss_weight'] + (d_real_aux_loss) * self.hparams['aux_loss_weight']
                ...
                g_total_loss = g_source_loss * self.hparams['src_loss_weight'] + d_fake_aux_loss * self.hparams['aux_loss_weight']
    ...
    def create_discriminator(self, input_shape=(8, 16, 1)):
        init = tfk.initializers.RandomNormal(stddev=0.02)
        const = tfk.constraints.max_norm(1.0)
        model_list = []
        in_image = tfkl.Input(shape=input_shape)
        ...
        out_aux = tfkl.Dense(self.hparams['pitches'], activation='softmax')(d)
    ...
    def create_generator(self, in_dim=(8, 16)):
        init = tfk.initializers.RandomNormal(stddev=0.02)
        const = tfk.constraints.max_norm(1.0)
        model_list = []
