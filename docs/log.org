...
#+MACRO: AUDIO @@html: <audio style='width: 100%;' controls="controls" src="$1"></audio>@@
...
#+CAPTION: Training of Aux classifier, shows the Nash Equilibrium. The skips are due to pausing training to try generating.
#+NAME: fig:class_aux
[[./img/aux_class_training.png]]

Figure [[fig:class_aux]] shows that it does reach an equilibrium, which is where the
...
over-fitting on the dataset, which is not idea either.
...
Figures [[fig:deep_gen_loss]] and [[fig:deep_disc_loss]] show a very unstable network
...
#+CAPTION:Later images at step count ~xxK
#+NAME: fig:shallow_spec_late
[[./img/shallow_spec_mid.png]]
#+end_columns

The loss graphs in figures [[fig:shallow_gen_loss]] and [[fig:shallow_disc_loss]] look
much better than before. Not visualised in them is that the the discriminator
starts from ~10, while the generator stars from ~40. This is because of the
weighted aux loss described earlier. Figure [[fig:shallow_spec_early]] shows to us
almost random noise but at this point almost all loss from the aux is
eliminated. At around step ~40K the generator starts generating images that
resembles spectrograms, shown in figure [[fig:shallow_spec_mid]].
